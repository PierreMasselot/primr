...
y <-  2 * x[,1] + 5 * x[,2] + 10 * (x[,1] >= .8 & x[,2] >= .5) + rnorm(100)
...
plot_trajectory(peel_res, type = "b", pch = 16, col = "cornflowerblue", 
  support = 0.1, abline.pars = list(lwd = 2, col = "indianred"), 
  xlab = "")
plot_trajectory(peel_res, type = "b", pch = 16, col = "cornflowerblue",
  support = 0.1, abline.pars = list(lwd = 2, col = "indianred"), 
  ytype = "rel.diff")
...
As expected, the peeling trajectory shows that the box's mean reaches a plateau 
when the support is lower than 0.1 (identified by the vertical line). 
This is also shown by a large relative difference in the bottom panel.

If we plot the corresponding box on the data, we see that this box fits well
the data.

```{r, fig.dim = c(7, 7)}
plot_box(peel_res, pch = 16, ypalette = hcl.colors(10), support = 0.1, 
  box.args = list(lwd = 2))
``` 
